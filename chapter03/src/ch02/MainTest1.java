...
public class MainTest1 {
	...
	public static void main(String[] args) {
		...
		// index 값이 3번째 녀석이면 자신의 타입을 화면에 출력해 주세요.
		for (int i = 0; i < remoteControllers.length; i++) {
			if (i == 3) {
				System.out.println("3번째 녀석은 ToyCar 데이터 타입 입니다.");
				//System.out.println(remoteControllers[i] instanceof RemoteController);
			}
		}
		
		//문제-3
		//만약 데이터 toyRobot 이면 멤버변수 name값을 출력해주세요
		for (int i = 0; i < remoteControllers.length; i++) {
			if (remoteControllers[i] instanceof ToyRobot) {
				//부모타입으로 업캐스팅된 상태이기 때문에
